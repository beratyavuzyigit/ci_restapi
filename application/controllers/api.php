...
class api extends CI_Controller
{
	public function product()
	{
		...
		if ($method == "get") { // Check if HTTP Method is "GET"
			...
			if (isset($params[1])) {      // Check if Exists {id} Parameter
				$id = $params[1];         // Assign {id} Parameter to Variable
				if (isset($params[2])) {  // Check if Exists {column_name} Parameter
					$column = $params[2]; // Assign {column_name} Parameter to Variable

					$result = $this->restapi_product->with_select($column, $id);	  // Get Product {column_name} Info by {id} 
					...
					$result = $this->restapi_product->where_id($id); 				  // Get Oroduct Info by {id}
					$response = $this->restapi_formatter->convert_to_format($result); // Product Data Convert to X Format
				...
				$result = $this->restapi_product->get_all(); 					  // Get All Product Info.
				$response = $this->restapi_formatter->convert_to_format($result); // Product Data Convert to X Format
			...
			$result = $this->restapi_product->create_product($product_data); // Send Created Product Array to Product Model
			...
			$result = $this->restapi_product->update_product($update_list); // Send Created Update List Array to Product Model
			...
			$params = explode('/', str_replace('api/product', '', uri_string())); // Get Parameters
			if (isset($params[1])) { 		// Check if Exists {id} Parameter
				$product_id = $params[1];	// Get {id} Parameter

				$this->load->model('restapi_product'); // Load Product Model

				$result = $this->restapi_product->delete_product($product_id); // Send {id} Parameter to Product Model

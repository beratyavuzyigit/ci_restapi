...
class restapi_product extends CI_Model
{
    ...
    */
    ...
    public function get_all($table_name)
    {
        ...
        $result = $this->db->get($table_name)->result_array();   // Get Data as Multi-Dimensional Array
    ...
    public function where_id($table_name, $id)
    {
        ...
        $result = $this->db->get($table_name)->row_array();  // Get Data as Array
        return $result;                                      // Return Data as Array
    }

    /* Get Product {column_name} İnfo by {id}  */
    public function with_select($table_name, $select_query, $id)
    {
        $this->load->database();                                    // Load Database
    ...
    public function create_product($product_data, $table_name)
    {
        $this->load->database();                                        // Load Database
        $this->db->where('stock_code', $product_data["stock_code"]);    // Get Data by {stock_code}
        $num_rows = $this->db->get($table_name)->num_rows();            // Get Query Row Count
        if ($num_rows) {                                                // Check if Exists {stock_code} in Products
            return false;                                               // ** Return False
        } else {
            $this->db->insert($table_name, $product_data);              // Create New Data in Products
            return true;                                                // ** Return True
        }
    }

    /* Update a Product by {id} */
    public function update_product($product_data, $table_name)
    {
        $this->load->database();                              // Load Database
        ...
        if ($num_rows) {                                      // Check if Exists {stock_code} in Products
            ...
            $this->db->update($table_name, $product_data);    // Update a Product Data
            return true;                                      // ** Return True
        } else {
            return false;                                     // ** Return false
        }
    ...
    public function delete_product($product_id, $table_name)
    {
        $this->load->database();                             // Load Database
        ...
        if ($num_rows) {                                     // Check if Exists {id} in Products
            ...
            $this->db->delete($table_name);                  // Delete a Product Data
            return true;                                     // ** Return True
        } else {
            return false;                                    // ** Return False
        }
